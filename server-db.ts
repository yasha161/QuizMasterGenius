...
export interface DbQuestion {
  id: string; // hash of normalized question text to prevent duplicate questions
  subject: string;
  topic: string;
  question: string;
  options: string[];
  ...
  createdAt: string;
  // Extended fields for custom Firestore/SQL schema
  questionId?: string;
  quizId?: string;
  subjectId?: string;
...
// Helper to split bilingual explanations
...
// --- DATABASE CONNECTION SETUP (PG vs LOCAL FALLBACK) ---
...
function getPoolConfig(url: string): pg.PoolConfig {
  // Parse postgresql://postgres:[12345678@Yash#123]@db.dtaxftdfvrixelafdjfe.supabase.co:5432/postgres
  ...
  // Fallback for standard strings or strings with password not bracketed but containing @
  // Since pg pool doesn't parse complex passwords in connection strings well, try a basic split if brackets are omitted
  try {
...
// Initialize tables in PostgreSQL if active
async function initPgTables() {
  if (!pgPool || !usePostgres) return;

  const client = await pgPool.connect();
  try {
    console.log("[Database] Initializing database tables in Supabase PostgreSQL...");
    ...
    console.log("[Database] Supabase PostgreSQL tables verified and active.");
  } catch (err) {
...
export async function ensureDefaultSubjectsExist() {
  ...
  if (usePostgres && pgPool) {
    try {
      ...
      console.error("[Database] Seeding default subjects into PostgreSQL failed, trying local fallback:", err);
    }
  }
      ...
      try {
        await pgPool.query(
          `INSERT INTO questions (
            id, subject, topic, question, options, correct_option_index, explanation, 
            difficulty, importance, question_id, quiz_id, subject_id, 
            correct_answer, explanation_english, explanation_hindi, source_file, created_at
          ...
          [
            ...
            questionData.explanationEnglish,
            questionData.explanationHindi,
            questionData.sourceFile,
  ...
  if (usePostgres && pgPool) {
    try {
      const res = await pgPool.query(
        "SELECT question_id FROM question_history WHERE LOWER(user_id) = $1",
        [lowerUser]
      );
    ...
    try {
      ...
      await client.query(
        `INSERT INTO attempts (
          id, user_id, topic, subject, difficulty, score, total_questions, accuracy, time_spent, 
          timestamp, questions, answers, attempt_id, quiz_id, correct_answers, wrong_answers, time_taken, created_at
      ...
      for (const qId of attempt.questions) {
        await client.query(
          `INSERT INTO question_history (user_id, question_id, attempted_at)
           VALUES ($1, $2, $3)
           ON CONFLICT (user_id, question_id) DO NOTHING`,
          [lowerUser, qId, timestamp]
        );
      }
...
export async function getUserAttempts(userId: string): Promise<DbAttempt[]> {
  ...
  if (usePostgres && pgPool) {
    try {
      const res = await pgPool.query(
        "SELECT * FROM attempts WHERE user_id = $1 ORDER BY timestamp DESC",
        [userId]
      );
      return res.rows.map(row => ({
        id: row.id,
...
export async function queryMatchingQuestions(
  topicQuery: string,
  difficulty: "Easy" | "Medium" | "Hard",
  skipIds: Set<string>
): Promise<DbQuestion[]> {
  if (usePostgres && pgPool) {
    try {
      ...
      console.error("[Database] PostgreSQL queryMatchingQuestions query failed:", err);
    }
  }
  ...
  if (usePostgres && pgPool) {
    try {
      await pgPool.query(
        `INSERT INTO subjects (id, subject_id, name, category, icon, created_at)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [subjectData.id, subjectData.subjectId, subjectData.name, subjectData.category, subjectData.icon, subjectData.createdAt]
      );
      return subjectData;
    } catch (err) {
...
export async function renameSubject(id: string, name: string): Promise<DbSubject | null> {
  ...
  if (usePostgres && pgPool) {
    try {
      const res = await pgPool.query(
        `UPDATE subjects SET name = $1 WHERE id = $2 OR subject_id = $2 RETURNING *`,
        [name, id]
      );
...
export async function deleteSubject(id: string): Promise<boolean> {
  ...
  if (usePostgres && pgPool) {
    ...
    try {
      ...
      // Cascade delete any user attempts associated with any of the subject's quizzes first
      ...
      // Cascade delete question history for all questions in this subject
  ...
  if (dbData.subjects.length !== initialLen) {
    // Cascading deletes in fallback JSON DB
...
export async function saveQuiz(quiz: Omit<DbQuiz, "id" | "createdAt">): Promise<DbQuiz> {
  ...
  // Save questions and associate with quiz ID
  ...
  if (usePostgres && pgPool) {
    try {
      await pgPool.query(
        `INSERT INTO quizzes (
          id, subject_id, name, difficulty, published, questions, created_at, quiz_id, title, total_questions, status
...
export async function updateQuiz(id: string, updated: Partial<DbQuiz>): Promise<DbQuiz | null> {
  ...
  if (usePostgres && pgPool) {
    try {
      await pgPool.query(
        `UPDATE quizzes SET 
          subject_id = $1, name = $2, difficulty = $3, published = $4, 
          questions = $5, title = $6, total_questions = $7, status = $8
         WHERE id = $9 OR quiz_id = $9`,
        [
          currentQuiz.subjectId,
          currentQuiz.name,
          currentQuiz.difficulty,
          currentQuiz.published,
          JSON.stringify(currentQuiz.questions),
          currentQuiz.title,
...
export async function deleteQuiz(id: string): Promise<boolean> {
  ...
  if (usePostgres && pgPool) {
    ...
    try {
      ...
      // Cascade delete any user attempts associated with this quiz first to prevent key/constraint issues
      await client.query("DELETE FROM attempts WHERE quiz_id = $1", [id]);

      // Cascade delete question history for all questions in this quiz
...
export async function saveJob(jobId: string, jobData: any): Promise<void> {
  if (usePostgres && pgPool) {
    try {
      ...
      await pgPool.query(
        `INSERT INTO jobs (id, data, created_at)
         VALUES ($1, $2, $3)
         ON CONFLICT (id) DO UPDATE SET data = EXCLUDED.data`,
        [jobId, JSON.stringify(merged), new Date().toISOString()]
      );
      return;
    } catch (err) {
